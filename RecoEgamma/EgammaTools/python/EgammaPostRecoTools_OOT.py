...
def _setupEgammaPostRECOSequence(process,applyEnergyCorrections=False,applyVIDOnCorrectedEgamma=False,era="2017-Nov17ReReco",runVID=True,applyEPCombBug=False):
    ...
    phoSrc = cms.InputTag('ootPhotons')
    ...
    if applyEnergyCorrections:
        phoCalibSrc = cms.InputTag('ootPhotons',processName=cms.InputTag.skipCurrentProcess())
        ...
        process.ootPhotons = process.calibratedPhotons.clone(src=phoCalibSrc,
                                                             produceCalibratedObjs=True)

        process.egammaScaleSmearTask = cms.Task(process.ootPhotons
                                                )
    else:
        phoCalibSrc = phoSrc
        eleCalibSrc = eleSrc
        ...
        process.egammaScaleSmearTask = cms.Task(process.calibratedOOTPhotons
                                                )
    if runVID:
...
def _setupEgammaPostRECOSequenceMiniAOD(process,applyEnergyCorrections=False,applyVIDOnCorrectedEgamma=False,era="2017-Nov17ReReco",runVID=True,applyEPCombBug=False):
    ...
    process.egammaScaleSmearTask = cms.Task(process.calibratedPatPhotons,
                                            process.slimmedOOTPhotons
                                            )
